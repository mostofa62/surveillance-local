...
class DashboardController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        /*
        if(session('project_id')=="") session(["project_id"=>session('user')->project_id]);
        ...
        if( ( in_array(8,$access) || in_array(9,$access) ) && $project_id==8) {
            return redirect(session('access').'covic/listreportlab');
        }else{
