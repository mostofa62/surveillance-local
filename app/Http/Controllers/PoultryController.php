...
class PoultryController extends Controller
{
    ...
    public function show($id)
    {
        ...
        if(session('user')->project_id!=1)
            ...
            if (session('user')->id == $poultry->user_id) {
...
//            die;
    ...
    public function questionView($id){
        if(session('user')->project_id!=1)
            ...
            view()->share('breadcamps', array('Home' => url(session('access') . 'dashboard'), 'Poultry' => url(session('access') . 'poultry'), "Call Initiate Form" => url(session('access') . 'poultry/callInitiate'), "Question Form" => url(session('access') . 'poultry/question/' . $id."/view")));
            view()->share('id', $id);
            view()->share('question', PoultryQuestion::where('poultry_id', $id)->first());
            view()->share('poultry', $poultry);
        ...
        if(session('user')->project_id!=1)
            ...
            $poultry = Poultry::select(DB::raw('poultry_questions.*'),DB::raw('poultries.*'))
                ...
                if(!in_array(3,session('accesslist_id')))
                    $poultry=$poultry->where('poultries.user_id',session('user')->id);
                if ($request->name) {
