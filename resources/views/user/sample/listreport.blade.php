        ...
        <div class="col-md-12">
                                ...
                                <tr>
                                    <td>
                                        <div class="form-group">
                                            ...
                                            <button type="button" onclick="window.location.href = 'listreport'; "  class="btn btn-sm btn-danger pull-right">Clear</button>
                                        </div>
                                    </td>
                                </tr>
                                </tbody>
        	...
        	<table class="table table-hover color-bordered-table muted-bordered-table usertable" align="center">
	        	...
	        	<tr>
                    ...
                    <td id="s7{!! @$model->cskid !!}" style="background-color: {{ @$model->stage7 >0 ? $done : $notdone  }};color:#fff;border-right: 1px solid #fff;">
                        {{ @$model->stage7 > 0 ? "COMPLETED" : "PENDING" }} 
                        <span class="label label-danger mail{!! @$model->cskid !!}" style="display: {{ $model->stage7 > 0 ?  'inline':'none'  }};color:#fff;">{{ @$model->stage7 == 1 && isset($model->email) && $model->email !='null' ? "MAIL SEND" : "Text SEND" }}</span>
                    </td>
        ...
        function sendupdate(id){
            ...
            $.ajax({
                ...
                data: data,
                    ...
                    @endif
                if(result.success > 0){
                    $("#s7"+id).html("COMPLETED <span class='label label-danger'>MAIL SEND</span>");
                }else{
                    $("#s7"+id).html("COMPLETED <span class='label label-danger'>Text SEND</span>");
                }
            });
